...
def wait_job(job_id, tag):
    ...
    while True:
        ...
        print(
            f"[{tag}] status={job.get('status')} progress={job.get('progress')} step={job.get('current_step')}",
            flush=True,
        )
...
print('ASSET', asset['id'], asset['file_name'], flush=True)
...
if versions:
    ...
    print('IMPORT_GUIDE', (guide.get('content') or '')[:120], flush=True)

print('DONE PROJECT', project_id, flush=True)
